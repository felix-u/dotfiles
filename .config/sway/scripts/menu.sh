...
tofi-drun --font "$(wq fontmono)" --width 36% --height 50% --hint-font false \
    --background-color "$(wq foreground)" --text-color "$(wq background)" \
    --font-size 20 --prompt-text " λ " --outline-width 0 --border-width 4 \
    --border-color "$(wq color8)" --selection-color "$(wq foreground)" \
    --selection-background "$(wq background)" --selection-padding 16 \
